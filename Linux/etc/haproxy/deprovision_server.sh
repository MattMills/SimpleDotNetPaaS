...
if ! grep -Fq "server $SERVERNAME" /etc/haproxy/conf.d/02-be-asptest_$SITENAME.conf; then
	echo "FAIL server not provisioned"
	exit 2
fi

grep -Fv "server $SERVERNAME" /etc/haproxy/conf.d/02-be-asptest_$SITENAME.conf > /etc/haproxy/conf.d/02-be-asptest_$SITENAME.conf.new
rm -f /etc/haproxy/conf.d/02-be-asptest_$SITENAME.conf
mv /etc/haproxy/conf.d/02-be-asptest_$SITENAME.conf.new /etc/haproxy/conf.d/02-be-asptest_$SITENAME.conf
echo "PASS server deprovisioned"
